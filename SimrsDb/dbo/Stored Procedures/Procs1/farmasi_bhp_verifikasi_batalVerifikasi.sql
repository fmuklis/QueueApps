...
-- = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =  = 
CREATE PROCEDURE [dbo].[farmasi_bhp_verifikasi_batalVerifikasi]
...
BEGIN
	...
	IF EXISTS(SELECT 1 FROM dbo.farmasiMutasi WHERE idMutasi = @idMutasi AND idStatusMutasi <> 3/*Verifikasi Request*/)
		BEGIN
			SELECT 'Tidak Dapat Batal Verifikasi, Request BHP'+ b.caption AS respon, 0 AS responCode
	...
	ELSE
		BEGIN TRY
			...
			/*Transaction Commit*/
			COMMIT TRAN;
			SELECT 'Verifikasi Request BHP Berhasil Dibatalkan' AS respon, 1 AS responCode;
		END TRY
		BEGIN CATCH
			ROLLBACK TRAN;

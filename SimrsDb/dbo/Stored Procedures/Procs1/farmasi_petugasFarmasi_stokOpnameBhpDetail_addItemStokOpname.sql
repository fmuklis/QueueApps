...
-- =============================================
CREATE PROCEDURE [dbo].[farmasi_petugasFarmasi_stokOpnameBhpDetail_addItemStokOpname]
...
BEGIN
	...
	DECLARE @idRuangan tinyint = (SELECT idRuangan FROM dbo.farmasiStokOpname
									 WHERE idStokOpname = @idStokOpname);
	SET NOCOUNT ON;

    -- Insert statements for procedure here
	IF EXISTS(SELECT 1 FROM dbo.farmasiMasterObatDetail WHERE idRuangan = @idRuangan
	...
	ELSE
		BEGIN
			...
			SELECT 'Data Tambah Stok BHP Berhasil Disimpan' AS respon, 1 AS responCode;
		END
END
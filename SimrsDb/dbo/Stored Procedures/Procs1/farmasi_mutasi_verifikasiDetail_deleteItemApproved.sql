...
-- = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = 
CREATE PROCEDURE farmasi_mutasi_verifikasiDetail_deleteItemApproved
...
BEGIN
	...
	IF EXISTS(SELECT 1 FROM dbo.farmasiMutasi WHERE idMutasi = @idMutasi AND idStatusMutasi NOT IN(2,3))
		BEGIN
			SELECT 'Tidak Dapat Dihapus, Request Mutasi '+ b.caption AS respon, 0 AS responCode
	...
	ELSE IF EXISTS(SELECT 1 FROM dbo.farmasiStokOpnameDetail a INNER JOIN dbo.farmasiStokOpname b ON a.idStokOpname = b.idStokOpname
		...
		BEGIN
			SELECT 'Tidak Dapat Dihapus, Selesaikan Proses Stok Opname Pada Barang Ini' AS respon, 0 AS responCode;
		END
	...
	ELSE
		BEGIN TRY
			...
			SELECT 'Item Verifikasi Request Mutasi Berhasil Dihapus' AS respon, 1 AS responCode;
		END TRY
		BEGIN CATCH
			ROLLBACK TRAN;

...
-- =============================================
CREATE PROCEDURE [dbo].[farmasi_pemakaianInternal_entryPemakaianInternalDetail_deleteItem]
...
BEGIN
	...
	IF EXISTS(SELECT TOP 1 1 FROM dbo.farmasiStokOpnameDetail a INNER JOIN farmasiPenjualanDetail b ON a.idObatDetail = b.idObatDetail
		...
		BEGIN
			SELECT 'Tidak Dapat Disimpan, Selesaikan Proses Stok Opname Barang Farmasi' AS respon, 0 AS responCode;
		END
	...
	ELSE
		BEGIN TRY
			...
			/*Delete Item Pemakaian Internal*/
			...
			/*Respon Sukses*/
			Select 'Item Pemakaian Internal Berhasil Dihapus' AS respon, 1 AS responCode;
		END TRY
		BEGIN CATCH

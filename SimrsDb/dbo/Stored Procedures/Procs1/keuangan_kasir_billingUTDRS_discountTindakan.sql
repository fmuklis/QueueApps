...
-- =============================================
CREATE PROCEDURE [dbo].[keuangan_kasir_billingUTDRS_discountTindakan]
...
BEGIN
	...
	IF EXISTS(SELECT 1 FROM dbo.transaksiTindakanPasien a
					 INNER JOIN dbo.transaksiBillingHeader b ON a.idPendaftaranPasien = b.idPendaftaranPasien
						AND a.idJenisBilling = b.idJenisBilling AND b.idStatusBayar <> 1/*Menunggu Pembayaran*/
			   WHERE a.idTindakanPasien = @idTindakanPasien)
		BEGIN
			SELECT 'Tidak Dapat Disimpan, Biaya Tindakan / Perawatan Telah Dibayar' AS respon, 0 AS responCode;
		END
	ELSE
		BEGIN

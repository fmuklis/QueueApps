...
-- = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = 
CREATE PROCEDURE [dbo].[farmasi_bhp_verifikasiDetail_validasi]
...
BEGIN
	...
	IF EXISTS(SELECT 1 FROM dbo.farmasiMutasi WHERE idMutasi = @idMutasi AND idStatusMutasi <> 3/*Request Diproses*/)
		BEGIN
			SELECT 'Tidak Dapat Divalidasi, Request BHP'+ b.caption AS respon, 0 AS responCode
	...
	ELSE IF NOT EXISTS(SELECT TOP 1 1 FROM dbo.farmasiMutasiOrderItem a
						...
						WHERE a.idMutasi = @idMutasi)
		BEGIN
			SELECT 'Tidak Dapat Divalidasi, Item Mutasi BHP Belum Dientry' AS respon, 0 AS responCode;
		END
	ELSE
		BEGIN TRY
			/*Transaction Begin*/
			...
			/*Update Stok BHP Jika Sudah Ada Barang Yang Sama Sebelumnya*/
			UPDATE a
			...
			UPDATE dbo.farmasiMutasi
			   SET idStatusMutasi = 4/*Request Diterima*/
				  ,kodeMutasi = dbo.generate_kodeMutasiBhp(@tanggalDiserahkan)
			...
			SELECT 'Request BHP Berhasil Divalidasi' AS respon, 1 AS responCode;
		END TRY
		BEGIN CATCH
			ROLLBACK TRAN;

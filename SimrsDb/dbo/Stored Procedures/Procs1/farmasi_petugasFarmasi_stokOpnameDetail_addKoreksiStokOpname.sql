...
-- =============================================
CREATE PROCEDURE [dbo].[farmasi_petugasFarmasi_stokOpnameDetail_addKoreksiStokOpname]
...
BEGIN
	...
	ELSE IF EXISTS(SELECT 1 FROM dbo.farmasiStokOpnameDetail WHERE idStokOpname = @idStokOpname AND idObatDetail = @idObatDetail)
		BEGIN
			SELECT 'Tidak Dapat Disimpan, Item Koreksi Stok Telah Terdaftar' AS respon, 0 AS responCode;
		END
	ELSE
		BEGIN
			...
			SELECT 'Data Koreksi Stok Barang Farmasi Berhasil Disimpan' AS respon, 1 AS responCode;
		END
END
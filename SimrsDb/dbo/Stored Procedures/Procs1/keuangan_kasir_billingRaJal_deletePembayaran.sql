...
-- =============================================
CREATE PROCEDURE [dbo].[keuangan_kasir_billingRaJal_deletePembayaran]
...
BEGIN
	...
	IF EXISTS(SELECT 1 FROM dbo.transaksiBillingPembayaran a
		...
		BEGIN
			SELECT 'Tidak Dapat Dihapus, Billing Rawat Jalan Telah Dibayar' As respon, 0 As responCode;
		END
	ELSE
		BEGIN
			...
			SELECT 'Data Pembayaran Billing Rawat Jalan Berhasil Dihapus' AS respon, 1 AS responCode;
		END
END
...
-- = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =  = 
CREATE PROCEDURE farmasi_mutasi_verifikasi_batalValidasi
...
BEGIN
	...
	IF EXISTS(SELECT 1 FROM dbo.farmasiMutasi WHERE idMutasi = @idMutasi AND idStatusMutasi <> 4/*Request Diterima*/)
		BEGIN
			SELECT 'Tidak Dapat Batal Validasi, Request Mutasi'+ b.caption AS respon, 0 AS responCode
	...
	ELSE IF EXISTS(SELECT TOP 1 1 FROM dbo.farmasiMutasiOrderItem a
		...
		BEGIN
			SELECT 'Tidak Dapat Batal Validasi, Item Mutasi Telah Terjual' AS respon, 0 AS responCode;
		END
	...
	ELSE
		BEGIN TRY
			...
			SELECT 'Penyerahan Barang Farmasi Dibatalkan' AS respon, 1 AS responCode;
		END TRY
		BEGIN CATCH
			ROLLBACK TRAN;

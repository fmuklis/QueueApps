...
-- = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = 
CREATE PROCEDURE farmasi_mutasi_requestMutasiDetail_validasi
...
BEGIN
	...
	ELSE IF NOT EXISTS(SELECT TOP 1 1 FROM dbo.farmasiMutasiOrderItem WHERE idMutasi = @idMutasi)
		BEGIN
			SELECT 'Tidak Dapat Divalidasi, Item Request Mutasi Belum Dientry' AS respon, 0 AS responCode;
		END
	ELSE
		BEGIN
			UPDATE [dbo].[farmasiMutasi]
			   SET idStatusMutasi = 2/*Request Mutasi Divalidasi*/
			 WHERE idMutasi = @idMutasi;

			SELECT 'Data Item Permintaan Mutasi Berhasil Divalidasi' AS respon, 1 AS responCode;
		END
END
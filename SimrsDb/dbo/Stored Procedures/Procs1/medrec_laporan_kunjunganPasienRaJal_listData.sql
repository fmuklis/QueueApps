...
-- = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = 
CREATE PROCEDURE [dbo].[medrec_laporan_kunjunganPasienRaJal_listData]
...
BEGIN
	...
	SET @query = 'SELECT tglDaftarPasien, noRM, namaPasien, namaJenisKelamin, tglLahirPasien, umur, namaJenisPenjaminInduk, diagnosaAwal, NamaOperator, namaRuangan, statusPasien
				    ...
				    FROM (Select a.tglDaftarPasien, b.noRM, b.namaPasien, b.namaJenisKelamin, b.tglLahirPasien, b.umur, da.namaJenisPenjaminInduk
								,g.diagnosa As diagnosaAwal, e.NamaOperator, f.namaRuangan, c.namaStatusPasien
						   ...
						   Where a.idJenisPendaftaran = 2/*RaJal*/ And a.idJenisPerawatan = 2/*RaJal*/ And a.idStatusPendaftaran > 1/*Sudah Diterima*/ And Convert(date, a.tglDaftarPasien) Between @tglAwal And @tglAkhir @dynamicHere) As dataSet
						  @filter
				ORDER BY namaJenisPenjaminInduk, namaRuangan, NamaOperator, tglDaftarPasien';

	SET @query = Replace(@query, '@dynamicHere', @where);

	SET @query = Replace(@query, '@filter', @filter);

	SET @paramDefinition = N'@tglAwal date, @tglAkhir date, @filter nvarchar(max)';

...
-- =============================================
CREATE PROCEDURE [dbo].[transaksiPendaftaranPasienSelectStatusCreateBillingRawatJalan]
...
BEGIN
	...
	/*SELECT idPendaftaranPasien, idBilling, idJenisBayar, tglDaftarPasien, noRM, namaPasien, tglLahirPasien, umur, namaRuangan
		  ,total - (((total - discTunai) * discPersen / 100) + discTunai) As total
	  FROM (SELECT b.idPendaftaranPasien, a.idBilling, a.idJenisBayar, b.tglDaftarPasien, bb.noRM, bb.namaPasien, bb.tglLahirPasien
				  ,bb.umur, ba.namaRuangan, c.biayaTindakan + IsNull(d.biayaBHP, 0) As total, a.discPersen, a.discTunai
			 ...
			 WHERE a.idJenisBilling = 1/*Billing Tagihan RaJal*/ And (a.tglBayar Is Null Or (Convert(date, a.tglBayar) = Convert(date, GetDate())))) As dataSet

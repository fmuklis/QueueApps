...
-- = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = 
CREATE PROCEDURE [dbo].[utdrs_pemeriksaan_entryTindakan_addPemeriksaan]
...
BEGIN
	...
	Declare @idRuangan int = (Select a.idRuangan From masterUser a Where a.idUser = @idUserEntry)
		   ...
		   ,@idJenisBilling int = CASE
									   WHEN EXISTS(SELECT 1 FROM dbo.transaksiPendaftaranPasien a
													WHERE a.idJenisPerawatan = 2/*RaJal*/ AND idJenisPendaftaran = 1/*IGD*/ AND a.idPendaftaranPasien = @idPendaftaranPasien)
											THEN 5/*Billing IGD*/
									   WHEN EXISTS(SELECT 1 FROM dbo.transaksiPendaftaranPasien a
													WHERE a.idJenisPerawatan = 2/*RaJal*/ AND idJenisPendaftaran = 2/*RaJal*/ AND a.idPendaftaranPasien = @idPendaftaranPasien)
											THEN 1/*Billing RaJal*/
									   ELSE 6/*Billing RaNap*/
								   END;
	SET NOCOUNT ON;
	...
	Else*/ 
		Begin Try
			...
			/*Transaction Commit*/
			Commit Tran;
			Select 'Data Tindakan / Pemeriksaan Berhasil Disimpan' AS respon, 1 AS responCode, @idTindakanPasien AS idTindakanPasien;	
		End Try
		Begin Catch
			Rollback Tran;

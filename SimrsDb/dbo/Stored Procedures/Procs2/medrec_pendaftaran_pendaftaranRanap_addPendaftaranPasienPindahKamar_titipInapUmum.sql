...
-- = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = 
CREATE PROCEDURE [dbo].[medrec_pendaftaran_pendaftaranRanap_addPendaftaranPasienPindahKamar_titipInapUmum]
	...
	ELSE		
		Begin Try
			...
			IF EXISTS(SELECT 1 FROM dbo.transaksiBillingHeader WHERE idPendaftaranPasien = @idPendaftaranPasien
				...
				BEGIN
					SELECT TOP 1 'Pindah Kamar Rawat Inap Berhasil, Arahkan Penanggung Jawab Pasien Untuk Menyelesaikan Pembayaran'+ b.namaJenisBilling AS respon, 0 AS responCode
			...
			ELSE
				BEGIN
					SELECT 'Pindah Kamar Rawat Inap Berhasil Disimpan' AS respon, 1 AS responCode;
				END
		End Try
		Begin Catch
